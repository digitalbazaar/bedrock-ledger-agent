...
const _getPeerId = callbackify(async ({ledgerNode}) => {
...
api.get = (actor, agentId, options, callback) => {
  ...
  async.auto({
    ...
    peerId: ['ledgerNode', (results, callback) => _getPeerId(
      {ledgerNode: results.ledgerNode}, callback)],
    createLedgerAgent: ['ledgerNode', 'peerId', (results, callback) => {
      const record = results.find;
      const laOptions = {
