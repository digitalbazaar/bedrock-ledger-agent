...
const brLedgerNodeGet = callbackify(brLedgerNode.get);
...
api.get = (actor, agentId, options, callback) => {
  ...
  async.auto({
    ...
    ledgerNode: ['checkPermission', (results, callback) => brLedgerNodeGet(
...
function _addNewLedgerNode(actor, options, callback) {
  async.auto({
    ...
    createLedgerNode: ['checkPermission', (results, callback) => {
      brLedgerNodeAdd(actor, options, callback);
    }]
  }, (err, results) => {
    if(err) {
      return callback(err);
...
function _addNewLedgerAgent(actor, ledgerNodeId, options, callback) {
  ...
  async.auto({
    ...
    ledgerNode: ['checkPermission', 'plugins', (results, callback) =>
      brLedgerNodeGet(actor, ledgerNodeId, options, callback)],
