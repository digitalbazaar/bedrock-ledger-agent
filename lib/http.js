      ...
      get: (req, res, callback) => {
        ...
        async.auto({
          ...
          privateLedgerAgents: ['publicLedgerAgents', (results, callback) => {
            ...
            // we only check for user.identity so this
            // is backwards compatible with other projects
            const actor = req.user.actor || req.user.identity;
            // req.query.owner could be a string or an array
            const owners = [].concat(
              req.query.owner ||
              (req.user.identity && req.user.identity.id) ||
