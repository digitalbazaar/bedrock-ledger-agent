...
bedrock.events.on('bedrock-express.configure.routes', app => {
    ...
    brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ledgerAgent: callback => brLedgerAgent.get(
          (req.user) ? req.user.identity : null, agentId, options, callback),
  ...
  app.post(routes.config, optionallyAuthenticated,
    brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ...
        add: ['getLedgerAgent', (results, callback) => {
          ...
          ledgerAgent.node.config.change(
  ...
  docs.annotate.post(routes.config, {
    description:
      'Request that a Ledger Agent make a configuration change to a ledger',
    schema: 'services.ledger.postLedgerConfig',
    securedBy: ['null'],
    responses: {
      204: 'Ledger configuration was accepted for writing.',
  ...
  app.post(routes.operations, optionallyAuthenticated,
    brRest.when.prefers.ld, (req, res, next) => {
      const actor = req.user ? req.user.identity : undefined;
      const agentId = 'urn:uuid:' + req.params.agentId;
      const options = {};
      ...
      async.auto({
        ...
        add: ['getLedgerAgent', (results, callback) => {
          ...
          ledgerAgent.node.operations.add({operation: req.body}, callback);
        }]
      }, err => {
        if(err) {
          return next(err);
  ...
  docs.annotate.post(routes.operations, {
    description:
      'Request that a Ledger Agent add an operation to perform to a ledger',
    schema: 'services.ledger.postLedgerOperation',
    securedBy: ['null'],
    responses: {
      204: 'Operation was accepted for writing.',
      400: 'Request failed due to malformed request.',
      403: 'Request failed due to invalid digital signature',
      409: 'Request failed due to duplicate information.'
    }
  ...
  app.get(routes.events, optionallyAuthenticated, brRest.when.prefers.ld,
    brRest.linkedDataHandler({
  ...
  docs.annotate.get(routes.events, {
    ...
    responses: {
      ...
      400: 'Bad Request. Unauthorized access is denied.',
      ...
      get: (req, res, callback) => {
        ...
        async.auto({
          ...
          getBlocks: ['getLedgerAgent', (results, callback) => {
            ...
            async.auto({
              genesis: callback =>
                ledgerAgent.node.blocks.getGenesis(callback),
              latest: callback =>
                ledgerAgent.node.blocks.getLatest(callback)
            }, (err, results) => {
              if(err) {
                return callback(err);
              }
          ...
          getBlock: ['getLedgerAgent', (results, callback) => {
            ...
            ledgerAgent.node.blocks.get({blockId}, callback);
          }]
        }, (err, results) => {
          if(err) {
            return callback(err);
  ...
  docs.annotate.get(routes.blocks, {
    ...
    responses: {
      ...
      400: 'Bad Request. Unauthenticated access is denied.',
      404: 'Event was not found.'
    }
  });
  ...
  // FIXME: Do we want to enable unauthenticated state machine queries?
  app.post(routes.query, optionallyAuthenticated,
      ...
      get: function(req, res, callback) {
        ...
        async.auto({
          ...
          getObject: ['getLedgerAgent', (results, callback) => {
            ...
            console.log('ZZZZZZZZZz', objectId);
