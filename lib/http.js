...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(routes.events, optionallyAuthenticated, brRest.when.prefers.ld,
    brRest.linkedDataHandler({
      get: function(req, res, callback) {
        ...
        if(req.query.owner || req.user !== undefined) {
          options.owner = req.query.owner || req.user.identity.id;
        }
        ...
        async.auto({
          getLedgerAgent: callback => brLedgerAgent.get(
            (req.user !== undefined) ? actor : null,
            agentId, options, callback),
          getEvent: ['getLedgerAgent', (results, callback) => {
            const ledgerAgent = results.getLedgerAgent;

            if(req.user === undefined && !ledgerAgent.public) {
  ...
  app.get(routes.blocks, optionallyAuthenticated, brRest.when.prefers.ld,
    ...
    get: function(req, res, callback) {
      ...
      async.auto({
        getLedgerAgent: callback => brLedgerAgent.get(
          (req.user !== undefined) ? actor : null, agentId, options, callback),
