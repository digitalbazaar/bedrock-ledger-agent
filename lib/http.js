...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    (req, res, next) => {
      ...
      const options = {
        owner: req.body.owner || req.user.account.id
      };
      ...
      brLedgerAgent.add(
        req.user.account, ledgerNodeId, options, (err, ledgerAgent) => {
          if(err) {
            return next(err);
          }
    ...
    cors(), brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ledgerAgent: callback => brLedgerAgent.get(
          (req.user) ? req.user.account : null, agentId, options, callback),
  ...
  app.post(routes.config, optionallyAuthenticated, cors(),
    brRest.when.prefers.ld, (req, res, next) => {
      const actor = req.user ? req.user.account : undefined;
      const agentId = 'urn:uuid:' + req.params.agentId;
      const options = {};

      if(req.query.owner || req.user) {
        options.owner = req.query.owner || req.user.account.id;
      }
