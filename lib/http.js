    ...
    brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ...
        addEvent: ['getLedgerAgent', (results, callback) => {
          ...
          // FIXME: Should not be switching off of body.type
          if(req.body.type === 'CreateWebLedgerRecord' ||
            req.body.type === 'UpdateWebLedgerRecord') {
            event = {
             '@context': config.constants.WEB_LEDGER_CONTEXT_V1_URL,
              type: 'WebLedgerEvent',
              operation: [req.body]
            };
          } else if(req.body.type === 'WebLedgerConfigurationEvent') {
            event = req.body;
          }

          ledgerAgent.node.events.add(event, {}, callback);
        }]
      }, (err, results) => {
        if(err) {
          return next(err);
        ...
        const eventUrl = url.format({
          ...
          pathname: url.parse(
            results.getLedgerAgent.service.ledgerEventService).pathname,
          query: {
