...
const brLedgerAgentGet = promisify(brLedgerAgent.get);
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(routes.agents + '/:agentId', optionallyAuthenticated,
    cors(), brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ...
        latestConfig: ['ledgerAgent', (results, callback) => callbackify(
          results.ledgerAgent.ledgerNode.storage.events.getLatestConfig
            .bind(results.ledgerAgent.ledgerNode.storage.events))(callback)
        ]
      }, (err, results) => {
        if(err) {
          return next(err);
    ...
    brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ...
        add: ['getLedgerAgent', (results, callback) => {
          ...
          callbackify(ledgerAgent.ledgerNode.config.change.bind(
            ledgerAgent.ledgerNode.config))(
    ...
    brRest.when.prefers.ld, (req, res, next) => {
      ...
      async.auto({
        ...
        add: ['ledgerAgent', (results, callback) => {
          ...
          callbackify(ledgerAgent.ledgerNode.operations.add.bind(
            ledgerAgent.ledgerNode.operations))(
      ...
      get: (req, res, callback) => {
        ...
        async.auto({
          ...
          getEvent: ['getLedgerAgent', (results, callback) => {
            ...
            callbackify(ledgerAgent.ledgerNode.events.get.bind(
              ledgerAgent.ledgerNode.events))(eventHash, {}, callback);
          }]
        }, (err, results) => {
          if(err) {
            return callback(err);
  ...
  // GET /ledger-agent/{AGENT_ID}/peers
  // Get recommended peers
  app.options(routes.peers, cors());
  app.get(
    routes.peers, optionallyAuthenticated, cors(),
    asyncHandler(async (req, res) => {
      ...
      const ledgerAgent = await brLedgerAgentGet(
        req.user ? actor : null, agentId, options);
      const {ledgerNode} = ledgerAgent;

      // all peers marked as recommended will be returned
      // limiting the number of recommended peers is up to the consensus plugin
      // that marks peers as recommended
      const result = await ledgerNode.peers.getRecommended();

      res.json(result);
    }));

  // GET /ledger-agent/{AGENT_ID}/peers/{PEER_ID}
  // Get a peer by ID
  app.get(
    routes.peer, optionallyAuthenticated, cors(),
    asyncHandler(async (req, res) => {
      const {actor} = req.user || {};
      const agentId = 'urn:uuid:' + req.params.agentId;
      const options = {};
      ...
      const result = await ledgerNode.peers.get({id: req.params.peerId});
      ...
      get: (req, res, callback) => {
        ...
        async.auto({
          ...
          getBlocks: ['getLedgerAgent', (results, callback) => {
            ...
            async.auto({
              genesis: callback =>
                callbackify(ledgerAgent.ledgerNode.blocks.getGenesis.bind(
                  ledgerAgent.ledgerNode.blocks))(callback),
              latest: callback =>
                callbackify(ledgerAgent.ledgerNode.blocks.getLatest.bind(
                  ledgerAgent.ledgerNode.blocks))(callback)
            }, (err, results) => {
              if(err) {
                return callback(err);
          ...
          getBlock: ['getLedgerAgent', (results, callback) => {
            ...
            callbackify(ledgerAgent.ledgerNode.blocks.get.bind(
              ledgerAgent.ledgerNode.blocks))({blockId}, callback);
          }]
        }, (err, results) => {
          if(err) {
            return callback(err);
      ...
      get: (req, res, callback) => {
        ...
        async.auto({
          ...
          getRecord: ['getLedgerAgent', (results, callback) => {
            ...
            callbackify(ledgerAgent.ledgerNode.records.get.bind(
              ledgerAgent.ledgerNode.records))({recordId}, (err, result) => {
              if(err) {
                return callback(err);
              }
