 ...
 *   * owner - the owner of the ledger node and agent (default: actor).
...
api.add = (actor, ledgerNodeId, options, callback) => {
  const createOptions = _.defaultsDeep(options, {
    storage: 'mongodb',
    private: true,
    owner: (actor && !options.owner) ? actor.id : options.owner
  });
 ...
 *   * owner - if present, filter results by this owner. Otherwise, filter
 *       results by actor.
...
api.getAgentIterator = function(actor, options, callback) {
  async.auto({
    find: callback => {
      ...
      // if actor is provided, filter by the actor
      if(actor) {
        query['meta.owner'] = actor.id;
      }
      // if owner is provided, filter by the owner
      if(options.owner) {
      ...
      const promise = cursor.next().then(record => {
        ...
        return cursor.hasNext().then(hasNext => {
          ...
          return new Promise((resolve, reject) => {
            const options = {};
            api.get(actor, record.ledgerAgent.id, options, (err, ledgerAgent) =>
